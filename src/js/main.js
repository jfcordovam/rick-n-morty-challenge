...
module.exports = {
   ...
   main() {
      ...
      let characters = fetchResource('character'),
         episodes = fetchResource('episode'),
         locations = fetchResource('location');
      ...
      return Promise.all([characters, episodes, locations])
         .then((values) => { // values structure -> [[string, array of objects, float], ... ]

            let results = ['First challenge: char counter'],
               apiRunTime = 0;

            // *** First challenge: character counter ***
            values.forEach(([name, collection, time]) => {
